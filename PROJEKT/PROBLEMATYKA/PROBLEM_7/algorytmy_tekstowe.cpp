...
void wstaw(TrieWezel* korzen, const string& wzorzec) {
    ...
    wezel->wzorzec = wzorzec;
}

vector<tuple<int, int, string>> znajdzWTekscie(TrieWezel* korzen, const string& wyrazenie){
    vector<tuple<int, int, string>> dopasowania;
    ...
    for (int i = 0; i < wyrazenie.size(); i++) {
        ...
        while (poz < wyrazenie.size()) {
            ...
            if (wezel->czyKoniecSlowa) {
                dopasowania.emplace_back(i, poz - i + 1, wezel->wzorzec);
            }
            poz++;
        }
    }
